...
>
  ...
  {invoicePreviewData.map((row, index) => {
    const gstAmount = ((row.rate * row.qty * row.gst) / 100).toFixed(2);
    const totalAmount = (row.rate * row.qty).toFixed(2);

    return (
      <TableRow key={index}>
        <TableCell>{index + 1}</TableCell>
        <TableCell>{row.description || "N/A"}</TableCell>
        <TableCell>{row.hsn || "N/A"}</TableCell>
        <TableCell>{row.qty || "0"}</TableCell>
        <TableCell>{row.discount || "N/A"}</TableCell>
        <TableCell>{row.gst || "0"}%</TableCell>
        <TableCell>{row.rate || "0.00"}</TableCell>
